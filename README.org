...
** Findings
  ...
  - Contract implementation-specific bugs (C8), followed by C02, C10 are respectively the most difficult bug type to spot
    The easiest ones are C05, C14, C01
 ...
 -  What is the relationship between categories of bugs and different types of protocols?
    + Broken business logic (C07) are the most frequent bugs classification found in Dexes, Derivatives, Yield Agreggator, and Stablecoins
    + Arrays (C14) and Wrong Math (C06) are the second most common type of bugs found in Dexes
